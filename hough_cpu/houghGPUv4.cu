...
// DETAILS: based on houghGPUv1.cu, but with:
// factoring out Hdiv2, Wdiv2 out of CUDA kernel for loop
// seems about same or slower.  saving double values in register seems not any better
// recalculating double values?
// time = 9262367884 ns
...
__global__ void computationalkernel(uint8_t *d, uint8_t *ht, int W, int H, int stride, int bpp, int th)
{
    ...
    for(rho = 0; rho < th; rho++)
    {
        ...
        if ( theta < 45 || (theta > 135 && theta < 225) || theta > 315) {
            for(y = 0; y < H; y++) {
                double dx = Wdiv2 + (rho - (Hdiv2-y)*S)/C;
            ...
            for(x = 0; x < W; x++) {
                double dy = Hdiv2 - (rho - (x - Wdiv2)*C)/S;
