...
// if grid size is not entered, default is 72
...
static int grid;
__global__ void computationalkernel(uint8_t *d, uint8_t *ht, int W, int H, int stride, int bpp, int th_in)
{
    int rho, y, x, th;
    int theta = (threadIdx.x + blockIdx.x * blockDim.x)/2; // theta is based on grid/ block id
    double C = cos(RAD(theta));  // todo: call sincos instead?
    double S = sin(RAD(theta));
    
    if ((threadIdx.x % 2) == 0)
    {
        // 1st half of the the theta column
        rho = 0;
        th = th_in/2;
    }
    else
    ...
    // rho is already initialized above
    for(; rho < th; rho++)
    {
        uint32_t totalred = 0;
...
uint8_t *houghtransform(uint8_t *h_in, int *w, int *h, int *s, int bpp)
{
    ...
    // right now this spawns 1440 total kernels, for 360 values of theta
    computationalkernel <<<grid, (720/ grid)>>> (d_in, d_out, W, H, *s, bpp, th);
...
int main(int argc, char **argv)
{
    ...
    grid = 72; // must be a factor of 360 (we calculate using theta for every degree of 360 degs)
