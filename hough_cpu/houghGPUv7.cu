...
// GPU v7
// DETAILS: Needs an extra param: ./<bin name> <input file> <output file> <grid size>
// if grid size is not entered, default is 12 which is the default for v1
...
__global__ void computationalkernel(uint8_t *d, uint8_t *ht, int W, int H, int stride, int bpp, int th)
{
    ...
    {
        float C = cos(RAD(theta));  // todo: call sincos instead?
        float S = sin(RAD(theta));
        uint32_t totalred = 0;
        ...
        if ( theta < 45 || (theta > 135 && theta < 225) || theta > 315) {
            for(y = 0; y < H; y++) {
                float dx = W/2.0 + (rho - (H/2.0-y)*S)/C;
            ...
            for(x = 0; x < W; x++) {
                float dy = H/2.0 - (rho - (x - W/2.0)*C)/S;
